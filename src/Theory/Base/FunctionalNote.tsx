	...
	constructor(key, interval) {
        this.key = key;
				...
				if(interval.id !== INTERVALS.TT.id) {
					...
					this.accidental = this.relativePosition - ScaleDefinitions[0].intervals[this.absoluteDegree - 1].semitones;
					...
					this.name = DiatonicDegreeDefinitions[this.absoluteDegree - 1].id + this.getAccidentalString(this.accidental);
				} else {
					this.absoluteDegree = 0;
					this.relativePosition = (ScaleDefinitions[0].intervals[key.degree - 1].semitones + 6 + this.key.accidental) % 12;
					this.accidental = 0;
