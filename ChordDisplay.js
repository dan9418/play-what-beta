class ChordDisplay extends React.Component {
	...
	render = () => {
		let homePosition = BASE_NOTES[this.props.chordDef.degree - 1].positionInC;
		let notes = [];
		for(let i = 0; i < this.props.chordDef.intervals.length; i++) {
			...
			let relativeDegree = ((degree + this.props.chordDef.degree - 1) % 7) + 1;
			let octave = Math.floor((degree + this.props.chordDef.degree - 1) / 7);
			let position = homePosition + this.props.chordDef.accidental + interval.semitones;
