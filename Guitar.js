...
class String extends React.Component {
    ...
    getFrets = () => {
        let frets = [];
        for(let i = 0; i <= 12; i++) {
            let note = new Note({absolutePosition: this.props.openPosition + i, keyDef: this.props.keyDef, notes: this.props.notes});
            frets.push(e(Fret, {key: `fret-${i}`, note: note, open: (i === 0), displaySettings: this.props.displaySettings}, null));
        }
        return frets;
    }
...
class Guitar extends React.Component {
    ...
    getStrings = () => {
        return this.strings.map((string, index) => {
            return e(String, {key: `key-${index}`, openPosition: string.openPosition, keyDef: this.props.keyDef, notes: this.props.notes, displaySettings: this.props.displaySettings}, null);
        });
    }
